...
export default async function AdminSubscriptionsPage() {
  ...
  // Eksplisit pakai FK guru_id karena `subscriptions` punya 2 FK ke `profiles`
  // (guru_id & confirmed_by), kalau tidak disebut PostgREST error "more than one relationship".
  const { data: subscriptions } = await supabase
    .from("subscriptions")
    .select("*, profiles!subscriptions_guru_id_fkey(name, phone)")
